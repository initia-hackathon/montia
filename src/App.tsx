...
export function App() {
  ...
  const [resultInfo, setResultInfo] = useState<{ type: "win" | "lose"; text: string; sub: string } | null>(null)
  const [finalPick, setFinalPick] = useState(-1)
  const [allRevealed, setAllRevealed] = useState(false)

  // Shuffle state
  const [shuffleCarDoor, setShuffleCarDoor] = useState(() => Math.floor(Math.random() * 3))
  ...
  const fireConfetti = useCallback(() => {
    if (typeof window !== "undefined" && (window as any).confetti) {
      ...
      const frame = () => {
        ;(window as any).confetti({ particleCount: 3, angle: 60, spread: 55, origin: { x: 0, y: 0.6 } })
        ;(window as any).confetti({ particleCount: 3, angle: 120, spread: 55, origin: { x: 1, y: 0.6 } })
  ...
  const makeChoice = useCallback(
    (choice: "stay" | "switch") => {
      ...
      if (won) {
        ...
        setMessage(`Car was behind the ${["first", "second", "third"][carDoor]} door`)
        setHistory((h) => [{ round, choice: choice.toUpperCase(), won: true, pnl, bet: CURRENT_BET, multiplier }, ...h])
        fireConfetti()
      } else {
        setResultInfo({
          type: "lose",
        ...
        setMessage(`Car was behind the ${["first", "second", "third"][carDoor]} door`)
        setHistory((h) => [{ round, choice: choice.toUpperCase(), won: false, pnl: -CURRENT_BET, bet: CURRENT_BET, multiplier }, ...h])
      }
    },
  ...
  const getDoorClasses = (i: number) => {
    const classes = ["door"]
    if (i === revealedDoor && phase !== "BET") classes.push("revealed", "eliminated", "goat-door")
    if (allRevealed) {
    ...
    <div className="app-shell">
      ...
      <div className="top-bar-wrapper">
        <div className="top-bar">
          ...
          <img src="/logo.png" alt="MONTIA" className="logo" />
          <ConnectButton />
        </div>
      </div>
      ...
      <div className="main-layout">
        ...
        <div className={`left-panel ${showLeftPanel ? "" : "collapsed"}`}>
          <div>
            ...
            <ul className="rule-steps">
              <li className="rule-step">
                ...
                <div className="step-text">Pick one of 3 doors. Behind one is a car, behind the others are goats.</div>
              </li>
              <li className="rule-step">
                <div className="step-num">2</div>
                <div className="step-text">The host reveals a goat behind one of the doors you didn&apos;t pick.</div>
              </li>
              <li className="rule-step">
                <div className="step-num">3</div>
                <div className="step-text">Choose to stay with your door or switch to the other unopened door.</div>
              </li>
              <li className="rule-step">
                <div className="step-num">4</div>
                <div className="step-text">If your final door has the car, you win!</div>
              </li>
            </ul>
          </div>
          ...
          <div className="payout-section">
            <h4>Payouts</h4>
            <div className="payout-row"><span className="payout-label">Stay + Win</span><span className="payout-val">1.80x</span></div>
            <div className="payout-divider" />
            <div className="payout-row"><span className="payout-label">Switch + Win</span><span className="payout-val">1.35x</span></div>
            <div className="payout-divider" />
            <div className="payout-row"><span className="payout-label">Stay odds</span><span className="payout-val">33.3%</span></div>
            <div className="payout-divider" />
            <div className="payout-row"><span className="payout-label">Switch odds</span><span className="payout-val">66.6%</span></div>
          </div>

          <div className="tip-box">
            <strong>Pro tip:</strong> Switching wins 2/3 of the time. The math says always switch!
          </div>
        </div>
          ...
          <div className="doors-container">
            {[0, 1, 2].map((i) => {
              if (phase === "SHUFFLE") {
                ...
                const classes = ["door"]
                if (shuffleRevealed) {
                  classes.push("revealed", isCarDoor ? "car-door" : "goat-door")
                }
                return (
                  <div
                    key={i}
              ...
              return (
                <div key={i} className={getDoorClasses(i)} onClick={() => pickDoor(i)}>
                  <div className="door-mystery">?</div>
                  <div className="door-icon" />
                  <div className="door-label">{i + 1}</div>
                </div>
            ...
            {phase === "SWITCH" && (
              <>
                ...
                <div className="switch-controls" style={{ display: "flex" }}>
                  <button className="switch-btn" onClick={() => makeChoice("stay")}>Stay</button>
                  <button className="switch-btn" onClick={() => makeChoice("switch")}>Switch</button>
                </div>
              </>
            )}

            {phase === "RESULT" && (
              <button className="play-again-btn" style={{ display: "inline-block" }} onClick={resetGame}>
                Play Again
              </button>
            )}
          </div>
        ...
        <div className="right-panel">
          <div className="panel-header">
            <h3>Bets <span className="bet-count">{history.length}</span></h3>
            <button className="clear-btn" onClick={clearHistory}>Clear</button>
          </div>
          <div className="bets-list">
            {history.length === 0 ? (
              <div className="bets-empty">No bets yet</div>
            ) : (
              history.map((h, idx) => (
                <div key={idx} className={`bet-entry ${h.won ? "win-entry" : "lose-entry"}`}>
                  <div className="bet-entry-top">
                    <span className={`bet-result ${h.won ? "win-text" : "lose-text"}`}>
                      {h.won ? "WIN" : "LOSS"}
                    </span>
                    <span className="bet-multiplier">{h.multiplier.toFixed(2)}x</span>
                  </div>
                  <div className="bet-details">Round {h.round} &bull; {h.choice}</div>
                  <div className="bet-pnl">
                    <span className="amount">{h.bet} INIT </span>
                    <span className={h.won ? "pnl-win" : "pnl-lose"}>
                      {h.pnl >= 0 ? "+" : ""}{h.pnl.toFixed(2)} INIT
                    </span>
                  </div>
                </div>
              ))
